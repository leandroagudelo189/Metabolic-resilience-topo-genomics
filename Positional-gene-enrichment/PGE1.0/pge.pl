...
# Copyright (C) 2016 Roland Barriot
#
...
#   CNRS UMR 5100 - Laboratoire de Microbiologie et de Génétique Moléculaires
#   Bât. IBCG
...
$refDatasets{'refseq_peptide'} = new Dataset(name=>'refseq_peptide',loadMethod=>'load_ID_CHR_START_END_DESC',
...
use vars qw/ %opt /;
use Getopt::Std;
my $opt_string = 'hlgr:q:a:s:c:m:p:';
...
print STDERR << "EOF";
    ...
    usage: $0 [-hl] -q query_IDs [-r dataset] [-a alpha_threshold] [-f redundancy_filter [-s ratio]] [-c chromosome] [-m mapped_IDs]
	...
	-p path to datasets     : default $dataDir
...
my $queryFile = $opt{q};
# REFERENCE DATASET
$dataset = lc($opt{r}) if $opt{r};$dataset = lc("$dataset");
...
foreach my $c (values %chrs) {
	my $r=1;
	my %loc; # bug fix 2015-03-23
	foreach my $g (sort { $a->start <=> $b->start } @{$c->ids}) {
		if (!defined $ids_ranks{$g->mappedID}) {
			$g->rank($r);
			#${%{$c->location}}{$r}=$g; # bug fix 2015-03-23
			$loc{$r} = $g;              # bug fix 2015-03-23
	...
	$c->size($r-1);
	$c->location( \%loc);  # bug fix 2015-03-23
}
...
foreach my $c (values %chrs) {
	...
	my @aEnd;
	my $bedtmp = '';
	my $location = $c->location();  # bug fix 2015-03-23
	foreach my $r (sort {$a->pvalue <=> $b->pvalue} @{$c->sEnriched}) {
		#~ my $start = ${%{$c->location}}{$r->start}->start; # bug fix 2015-03-23
		#~ my $end   = ${%{$c->location}}{$r->end}->end;     # bug fix 2015-03-23
		my $start = $location->{$r->start}->start;  # bug fix 2015-03-23
		my $end   = $location->{$r->end}->end;  # bug fix 2015-03-23
		my $score = 'inf';
		if ($r->pvalue > 0) {
