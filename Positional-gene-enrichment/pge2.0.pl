#!/usr/bin/perl
# This file has been udated and modified from PGE by Katleen De Preter et al. Positional gene enrichment analysis of gene sets for high-resolution identification of overrepresented chromosomal regions
# pge version 1.02
...
sub filter_redundant_hits {
    foreach my $chr_obj (values %chrs) {
        ...
        # Remove non-pertinent hits
        $chr_obj->sEnriched([ grep { $_->pertinent } @{$chr_obj->sEnriched} ]);
    }
}
...
sub generate_output {
    ...
    foreach my $chr_obj (values %chrs) {
        ...
        foreach my $hit (@hits) {
            ...
            my $score = $hit->pvalue > 0 ? int(-3 * log($hit->pvalue)) : 'inf';
